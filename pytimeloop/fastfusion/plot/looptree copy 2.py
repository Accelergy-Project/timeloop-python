...
class Node(ABC):
    def __init__(self, this_level: Iterable[Any] = (), children: Iterable["Node"] = (), backing_stores: set[TensorStorage]=None):
    ...
    def get_backing_storage(self, _entry: bool = True, start_at: int = 0) -> list[TensorStorage]:
        if start_at <= 0:
            storage = set(t for t in self.this_level if isinstance(t, TensorStorage))
        else:
            storage = set()
        for c in self.children:
            storage.update(c.get_backing_storage(_entry=False, start_at=start_at - 1))
        return sorted(storage & self.backing_stores) if _entry else storage
    
    def get_backing_storage_beneath_loop(self) -> list[TensorStorage]:
        storage = set()
        for c in self.children:
            if isinstance(c, LoopNode):
                storage.update(c.get_backing_storage())
            else:
                storage.update(c.get_backing_storage_beneath_loop())
        return sorted(storage  & self.backing_stores)
    
    def get_backing_stores(self):
        return TensorStorage.get_backing_stores(self.get_backing_storage())
    ...
    def get_shared_tensors(self, other: "Node", start_at: int=0) -> set[TensorStorage]:
        return set(self.get_backing_storage(start_at=start_at)) & set(other.get_backing_storage(start_at=start_at))
    
    def insert(self, node: "Node", above_loop_index: int):
        if isinstance(self, LoopNode):
            above_loop_index -= 1
        
        if above_loop_index <= 0:
            node.children = self.children
            self.children = [node]
        else:
            self.children[-1].insert(node, above_loop_index)
            
    def insert_below_right(self, right: "Node"):
        right.children = merge_children(self.children, right.children)
        self.children = [right]
        return self
    
    def insert_below_left(self, left: "Node"):
        left.children = merge_children(left.children, self.children)
        self.children = [left]
        return self
    
    def merge(self, right: "Node") -> list["Node"]:
        if set(self.get_backing_storage_beneath_loop()) & set(right.get_backing_storage_beneath_loop()):
            return [self.fuse(right)]
        else:
            return [self, right]
        
    @abstractmethod
    def fuse(self, right: "Node") -> "Node":
        pass
    
    def __str__(self):
        this_level = str(self.this_level)
        children = [str(c) for c in self.children]
        children = '\t' + '\n\t'.join(c.replace('\n', '\n\t') for c in children)
        return f"{this_level}\n{children}"
        
def merge_children(left: list[Node], right: list[Node]) -> list[Node]:
    if not left:
        return right
    if not right:
        return left
    last_left = left[-1]
    first_right = right[0]
    middle = last_left.merge(first_right)
    return left[:-1] + middle + right[1:]

class RootNode(Node):
    def __init__(self, children: Iterable[Node] = (), *args, **kwargs):
        super().__init__(children, *args, **kwargs)
        
    def pydot_str(self):
        return "Root"
    
    def merge(self, right: "RootNode") -> list["RootNode"]:
        return [self.fuse(right)]

    def fuse(self, right: "RootNode") -> "RootNode":
        self.children = merge_children(self.children, right.children)
        return self

class LoopNode(Node):
    def __init__(self, loop: Loop, *args, **kwargs):
        super().__init__([loop], *args, **kwargs)
        self.loop = loop
        
    def fuse(self, right: "LoopNode") -> "LoopNode":
        if not isinstance(right, LoopNode):
            right.children = merge_children([self], right.children)
            return right
        assert isinstance(right, LoopNode)
        # assert self.loop == right.loop
        new_loop = self.loop.merge_next(right.loop)
        self.loop = new_loop
        self.children = merge_children(self.children, right.children)
        return self

class StorageNode(Node):
    def __init__(self, storage: TensorStorage, *args, **kwargs):
        super().__init__([storage], *args, **kwargs)

    def pydot_str(self):
        return f"{self.this_level[0].tensor_name} ({self.this_level[0].tile_size})"
    
    def fuse(self, right: "StorageNode") -> "StorageNode":
        if not isinstance(right, StorageNode):
            self.children = merge_children(self.children, [right])
            return self
        if self.this_level[0] == right.this_level[0]:
            self.children = merge_children(self.children, right.children)
            return self

        if self.this_level[0] in right.get_backing_storage():
            right.insert_below_left(self)
            return right
        if right.this_level[0] in self.get_backing_storage():
            self.insert_below_right(right)
            return self
...
class ComputeNode(Node):
    def __init__(self, compute: str, *args, **kwargs):
        super().__init__([compute], *args, **kwargs)

    def pydot_str(self):
        return f"{self.this_level[0]}"
    
    def fuse(self, right: "ComputeNode") -> "ComputeNode":
        raise ValueError("Cannot fuse ComputeNode")

class NotEnoughLoopsError(Exception):
    pass
    
    
def tiling2tree(einsum_id: str, tiling: Tiling, backing_stores: set[TensorStorage]) -> Node:
    base = RootNode(backing_stores=backing_stores)
    n = base
    for l in tiling.loops:
        n.children.append(LoopNode(l, backing_stores=backing_stores))
        n = n.children[-1]
    n.children.append(ComputeNode(f"Einsum {einsum_id}", backing_stores=backing_stores))
    for t in sorted(tiling.storage):
        try:
            base.insert(StorageNode(t, backing_stores=backing_stores), t.above_loop_index)
        except:
            raise NotEnoughLoopsError(f"Einsum {einsum_id} has a tensor {t.tensor_name} that requires more loops than are present in the tiling.")
    return base


def get_tensor_lifetimes(einsum_ids, mappings, still_live_tensors):
...
def tilings2looptree(mappings: dict[str, Tiling], stats: dict[str, Any], 
    ...
    tensors_lifetimes = get_tensor_lifetimes(einsum_ids, mappings, still_live_tensors)
    trees = {}
    
    all_tensors = set().union(*[set(t) for t in tensors_lifetimes.values()])
    backing_stores = set(TensorStorage.get_backing_stores(all_tensors))

    mappings_with_lifetimes = {}
    for k, v in mappings.items():
        mappings_with_lifetimes[k] = v.add_tensors(*tensors_lifetimes[k])
        trees[k] = tiling2tree(k, mappings_with_lifetimes[k], backing_stores)
    ...
    while n is not None:
        ...
        while i < len(n.this_level):
            t = n.this_level[i]
            if t in backing_stores and t.tensor_name in skip_backing_tensors_in_right_branch:
                n.this_level.pop(i)
            else:
                i += 1
...
#                 shared_tensors |= set(
#                     c for c in children[i].get_backing_storage(start_at=1) if c.tensor_name in still_live_tensors
#                 )
